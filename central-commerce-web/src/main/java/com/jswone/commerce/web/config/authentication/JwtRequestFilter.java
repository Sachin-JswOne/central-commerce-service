...
public class JwtRequestFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (authenticationMode == null) {
      ...
      response.sendError(
          HttpServletResponse.SC_BAD_REQUEST, "Authentication header is missing in the request");
      return;
    }
    ...
    try {
      ...
      if (Objects.nonNull(e.getMessage()) && e.getMessage().contains(TOKEN_EXPIRE_MESSAGE)
          || Objects.nonNull(e.getMessage()) && e.getMessage().contains(INVALID_TOKEN_MESSAGE)
  ...
  private void handleApiKeyAuthentication(HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    ...
    if (xApiKey.equals(commerceValueConfig.getX_API_KEY_COMMERCE_SERVICE())) {
      List<String> apiKeyParts =
          Arrays.asList(commerceValueConfig.getX_API_KEY_COMMERCE_SERVICE().split("-"));
  ...
  private void addTokenValuesToRequestAttributes(HttpServletRequest request, Map claims) {
    if (claims.containsKey(CORRELATION_ID_CLAIM)) {
      request.setAttribute(CORRELATION_ID_CLAIM, claims.get(CORRELATION_ID_CLAIM));
    }
  }
