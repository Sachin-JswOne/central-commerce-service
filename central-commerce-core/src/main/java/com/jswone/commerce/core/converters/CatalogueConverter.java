...
public class CatalogueConverter {
  ...
  public SearchResponse convertGenericSearchToSearchResponse(
    ...
    try {
      List<Product> products =
          Optional.ofNullable(productSearchResponse.getProducts()).orElse(Collections.emptyList());
      ...
      if (searchRequest.isSearchAction()) {
        ...
        List<PLPCard> plpCards =
            products.stream().map(this::convertToPLPCard).collect(Collectors.toList());
        ...
        List<SearchSuggestion> suggestionList =
            products.stream().map(this::convertToSuggestion).toList();
      ...
      log.error(
          "Exception while mapping central catalogue search response → PLP: {}", e.getMessage(), e);
  ...
  private List<ProductFilterConditions> buildDynamicFilters(
      List<Product> products, SearchRequest searchRequest) {
    ...
    for (Product p : products) {
      ...
      for (var entry : attrs.entrySet()) {
        ...
        selectionValues.computeIfAbsent(key, x -> new TreeSet<>()).add(String.valueOf(val));
      }
    }
    ...
    if (searchRequest.getFilterConditions() != null) {
      for (ProductFilterConditions reqFilter : searchRequest.getFilterConditions()) {
        ...
        selectedFromRequestMap.put(
            ...
            reqFilter.getSelectedValues() == null
                ? Collections.emptyList()
                : reqFilter.getSelectedValues());
      }
    }

    selectionValues.forEach(
        (key, values) -> {
          List<String> selected =
              selectedFromRequestMap.getOrDefault(key.toLowerCase(), Collections.emptyList());
