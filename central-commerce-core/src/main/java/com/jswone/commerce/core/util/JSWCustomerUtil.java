...
public class JSWCustomerUtil {
  ...
  @Autowired
  @Qualifier("requestBuilderCTAdmin")
  ByProjectKeyRequestBuilder requestBuilderCT;
  ...
  public Customer getCustomerById(String customerId) {
    ...
    try {
      customerResponse =
          requestBuilderCT.customers().withId(customerId).get().executeBlocking().getBody();
    } catch (Exception e) {
  ...
  private Address getDefaultShippingAddress(Customer customer) {
    return customer.getAddresses().stream()
        ...
        .filter(address -> address.getId().equalsIgnoreCase(customer.getDefaultShippingAddressId()))
        .findFirst()
        .orElse(null);
  }
