...
public class ProductServiceImpl implements ProductService {
  ...
  public ProductServiceImpl(
      ProductCatalogueStoreRepository productCatalogueStoreRepository,
      ProductAttributeUtil productAttributeUtil) {
  ...
  public SkuInfo getMatchedVariantResponse(ProductSkuRequest productSkuRequest) {
    ...
    List<ProductAttributeDTO> newProductAttributeDTO =
        productAttributeUtil.convertAttributes(productAttributeDTOS);
    productSkuRequest =
        productSkuRequest.toBuilder().productAttributes(newProductAttributeDTO).build();
  ...
  private SkuInfo getMatchedVariant(ProductSkuRequest productSkuRequest) {
    try {
      ...
      productCatalogueStore =
          productCatalogueStoreRepository.findProductCatalogueStoresByProductMaterialMasterId(
              productSkuRequest.getProductMaterialMasterId());
      ProductSelectorSkuResponse productSkuRes =
          Objects.nonNull(productCatalogueStore)
              ...
              ? getVariant(productCatalogueStore, productSkuRequest.getProductAttributes())
              : null;
      ...
      throw new ProductSelectorException(
          productSkuRequest, e.getLocalizedMessage(), HttpStatus.BAD_GATEWAY);
    }
  }
  ...
  public ProductSelectorSkuResponse getVariant(
    ...
    if (productCatalogueStore.getVariants() != null
      ...
      for (Variant variant : productCatalogueStore.getVariants()) {
        String variantMMId =
            StringUtils.defaultIfEmpty(variant.getMmId(), JSWGenericConstants.EMPTY_STRING);

        if (variantMMId.equals(JSWGenericConstants.EMPTY_STRING)) {
          variantMMId =
              productCatalogueStore.getProductMaterialMasterId().concat("-").concat("10000000");
        }
      ...
      log.error("No variants found for product key: {}", productCatalogueStore.getProductKey());
    }
  ...
  public static Pair<Boolean, List<Attribute>> isMatchProductVariant(
    ...
    boolean productFlag =
        attributes.stream()
            .allMatch(a -> variant.getAttributes().stream().anyMatch(va -> checkEquality(va, a)));
  ...
  protected static Pair<Boolean, List<Attribute>> checkForRangeBasedMatch(
    ...
    attributes.forEach(
        val -> {
          ...
          if (lastIndexOf == -1) {
            flatList.add(variant.getAttributes().stream().anyMatch(va -> checkEquality(va, val)));
            return;
          }
          ...
          variant
              ...
              .forEach(
                  va -> {
                    if (va.getName().equals(minAttr) || va.getName().equals(otherMinAttr)) {
                      minAttribute.setKey(va.getName());
                      minAttribute.setValue(va.getValue());
                    } else if (va.getName().equals(maxAttr) || va.getName().equals(otherMaxAttr)) {
          ...
          if (minAttribute.getValue() != null
              ...
              && Comparable.class.isAssignableFrom(maxAttribute.getValue().getClass())) {
  ...
  private static boolean checkEquality(Attribute va, Attribute a) {
